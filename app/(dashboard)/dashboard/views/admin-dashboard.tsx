...
import {
  ...
  Truck, Phone, Layers, Target, CheckCircle2, ChevronRight, ShoppingCart,
} from "lucide-react";
import {
...
export async function AdminDashboard({ userName }: { userId: string; userName: string }) {
  const [
    stats, revenueData, activities, urgentOrders,
    capacity, nextDelivery, heatmap, todayAppts, readyOrders,
  ] = await Promise.all([
    getDashboardStats(), getRevenueData(), getRecentActivities(),
    getUrgentOrders(), getWorkshopCapacity(), getNextDelivery(),
    getBookingHeatmap(), getTodayAppointments(), getReadyOrders(),
  ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-[#D4AF37]/20 bg-gradient-to-br from-[#0a0a0a] via-[#111] to-[#1a1500] min-h-[160px] flex items-center">
        <div className="absolute inset-0 opacity-10" style={{ backgroundImage: "repeating-linear-gradient(45deg,#D4AF37 0,#D4AF37 1px,transparent 0,transparent 50%)", backgroundSize: "20px 20px" }} />
        <div className="relative z-10 flex items-center justify-between w-full px-8 py-6">
          <div>
            ...
            <p className="text-white/50 text-sm mt-1">{now.toLocaleDateString("en-AE", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}</p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="border-border/50">
          <CardContent className="p-5">
            ...
            <div className="mt-3 h-1.5 bg-secondary rounded-full overflow-hidden">
              <div className="h-full rounded-full bg-[#D4AF37]" style={{ width: `${capacity.percentage}%` }} />
            </div>
        ...
        <Card className="border-border/50">
          <CardContent className="p-5">
            ...
            {nextDelivery ? (
              <>
                <p className="font-semibold">{(nextDelivery as any).customer?.name}</p>
                <p className="text-xs text-muted-foreground mt-0.5">{(nextDelivery as any).garmentType}</p>
                <p className="text-lg font-bold text-[#D4AF37] mt-2">{new Date((nextDelivery as any).deliveryDate).toLocaleDateString("en-AE", { day: "2-digit", month: "short" })}</p>
                <p className="text-xs text-muted-foreground">{(nextDelivery as any).orderNumber}</p>
              </>
              ...
              {[1, 2, 3, 4, 5, 6, 0].map((dayIdx, i) => {
                ...
                return (
                  <div key={i} className="flex-1 flex flex-col items-center gap-1">
                    <div className="w-full rounded-sm" style={{ height: `${Math.max(8, intensity * 48)}px`, backgroundColor: `rgba(212,175,55,${0.1 + intensity * 0.9})` }} />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Appointments Today", value: stats.todayAppointments, sub: "requiring attention", icon: <Calendar className="w-4 h-4 text-blue-400" />, bg: "bg-blue-500/10" },
          ...
          { label: "Revenue This Month", value: null, currency: stats.totalRevenue, sub: "paid invoices", icon: <DollarSign className="w-4 h-4 text-[#D4AF37]" />, bg: "bg-[#D4AF37]/10" },
        ].map((s, i) => (
      ...
      {urgentOrders.length > 0 && (
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-semibold text-amber-400">{urgentOrders.length} order{urgentOrders.length !== 1 ? "s" : ""} due within 7 days</span>
          </div>
          <div className="flex flex-wrap gap-2">
            {urgentOrders.map((o: any) => (
              <a key={o.id} href="/orders" className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-card border border-border hover:border-amber-500/40 text-xs transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card className="border-border/50">
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground uppercase tracking-wider">Today's Appointments</CardTitle></CardHeader>
          <CardContent>
            {todayAppts.length === 0 ? <p className="text-sm text-muted-foreground py-4 text-center">No appointments today</p> : (
              <ul className="space-y-2">
                {todayAppts.slice(0, 6).map((a: any) => (
                  <li key={a.id} className="flex items-center gap-3 py-2 border-b border-border/50 last:border-0">
                    <div className="w-8 h-8 rounded-lg bg-blue-500/10 flex items-center justify-center flex-shrink-0"><Calendar className="w-4 h-4 text-blue-400" /></div>
        ...
        <Card className="border-border/50">
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-muted-foreground uppercase tracking-wider">Orders Ready for Collection</CardTitle></CardHeader>
          <CardContent>
            {readyOrders.length === 0 ? <p className="text-sm text-muted-foreground py-4 text-center">No orders pending action</p> : (
              <ul className="space-y-2">
                {readyOrders.slice(0, 6).map((o: any) => (
                  <li key={o.id} className="flex items-center gap-3 py-2 border-b border-border/50 last:border-0">
                    <div className="w-8 h-8 rounded-lg bg-[#D4AF37]/10 flex items-center justify-center flex-shrink-0"><CheckCircle2 className="w-4 h-4 text-[#D4AF37]" /></div>
                    <div className="flex-1 min-w-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Card className="border-border/50">
          <CardHeader><CardTitle className="text-sm font-medium text-muted-foreground uppercase tracking-wider">Recent Activity</CardTitle></CardHeader>
          <CardContent>
            {activities.length === 0 ? <p className="text-sm text-muted-foreground text-center py-4">No recent activity</p> : (
      ...
      {/* Module glance */}
      <div>
        <h2 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-3">All Modules</h2>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <a href="/pos" className="block p-4 rounded-xl border border-border/50 bg-card hover:border-[#D4AF37]/40 transition-all">
          ...
          <a href="/leads" className="block p-4 rounded-xl border border-border/50 bg-card hover:border-blue-400/40 transition-all">
            ...
            <p className="text-[10px] text-muted-foreground">active · AED {stats.pipelineValue.toLocaleString("en-AE")}</p>
          </a>
          <a href="/fabrics" className="block p-4 rounded-xl border border-border/50 bg-card hover:border-purple-400/40 transition-all">
            ...
            <p className={`text-[10px] ${stats.lowStockFabrics > 0 ? "text-amber-400" : "text-muted-foreground"}`}>{stats.lowStockFabrics > 0 ? `${stats.lowStockFabrics} low stock` : "All stocked"}</p>
          </a>
          <a href="/followups" className="block p-4 rounded-xl border border-border/50 bg-card hover:border-green-400/40 transition-all">
            ...
            <p className={`text-[10px] ${stats.overdueFollowUps > 0 ? "text-red-400" : "text-muted-foreground"}`}>{stats.overdueFollowUps > 0 ? `${stats.overdueFollowUps} overdue` : "All on track"}</p>
          </a>
        </div>
      </div>
    </div>
