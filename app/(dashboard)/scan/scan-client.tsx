...
export function ScanClient({ userPosition, userName, userRole }: ScanClientProps) {
  ...
  const navigateToOrder = (raw: string) => {
    setError("");
    raw = raw.trim();
    if (!raw) return;
    let orderId = raw;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    navigateToOrder(value);
  };

  const handleCameraDetect = (data: string) => {
    setCameraOpen(false);
    ...
    <>
      ...
      <div className="max-w-md mx-auto px-4 py-8 space-y-8">
        ...
        <div className="space-y-3">
          <p className="text-sm font-semibold">Or enter Order ID manually</p>
          <form onSubmit={handleSubmit} className="flex gap-2">
            <Input
              ...
              spellCheck={false}
            />
            <Button type="submit" variant="outline" disabled={!value.trim()} className="gap-1.5 shrink-0">
        ...
        <div className="rounded-xl border border-border bg-secondary/20 p-4 space-y-3">
          ...
          <ol className="space-y-2 text-xs text-muted-foreground list-decimal list-inside">
            <li>Tap "Open Camera to Scan" above</li>
            <li>Point the camera at the printed order QR code</li>
